             ...
             const char *units, time_t epoch_time, pcmk__output_t* out)
{
    ...
    if (value) {
        switch (output_format) {
            ...
            case mon_output_cgi:
                if (out) {
                    char date_str[LINE_MAX];
                    snprintf(date_str, LINE_MAX, "%s%s", value, (units? units : ""));
...
print_resources_closing(FILE *stream, gboolean printed_heading, pcmk__output_t *out)
{
    ...
    switch (output_format) {
        ...
        case mon_output_cgi:
            if (!printed_heading) {
                if (out) {
                    char sresources[LINE_MAX];
                    snprintf(sresources, LINE_MAX, "No %sresources", heading);
                    out->list_item(out, "hr", NULL);
        ...
        case mon_output_cgi:
            ...
            if (out) {
                ...
                if (data_set->disabled_resources || data_set->blocked_resources) {
                    ...
                    if (data_set->disabled_resources) {
                        snprintf(str, LINE_MAX, "%d", data_set->disabled_resources);
                        out->list_item(out, NULL, str);
                    ...
                    if (data_set->blocked_resources) {
                        snprintf(str, LINE_MAX, "%d", data_set->blocked_resources);
                        out->list_item(out, NULL, str);
        ...
        case mon_output_cgi:
            {
                ...
                case st_done:
                    ...
                    break;
                case st_failed:
                    snprintf(str, LINE_MAX, "%s of %s failed: delegate=%s, "
                            "client=%s, origin=%s, %s='%s'",
                            action_s, event->target,
                            event->delegate ? event->delegate : "",
                            event->client, event->origin,
                    ...
                    break;
                default:
                    snprintf(str, LINE_MAX, "%s of %s pending: client=%s, "
                            "origin=%s",
                            action_s, event->target,
                            event->client, event->origin);
                }
    ...
    for (gIter = data_set->nodes; gIter != NULL; gIter = gIter->next) {
        ...
        if (group_by_node) {
            ...
            for (lpc2 = node->details->running_rsc; lpc2 != NULL; lpc2 = lpc2->next) {
                ...
                rsc->fns->print(rsc, "            ", print_opts | pe_print_rsconly, stream, out);
            }
        }
        out->end_list(out);
    }
                  ...
                  stonith_history_t *stonith_history)
{
    ...
    for (gIter = data_set->nodes; gIter != NULL; gIter = gIter->next) {
        ...
        if (node->details->standby_onfail && node->details->online) {
            ...
            snprintf(str, LINE_MAX, "standby%s", node->details->running_rsc

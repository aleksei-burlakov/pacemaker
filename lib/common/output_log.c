...
log_err(pcmk__output_t *out, const char *format, ...) {
    ...
    pcmk__output_crm_log(out, buffer);
...
int pcmk__output_get_log_level(pcmk__output_t *out) {
    CRM_ASSERT(out && out->priv);
    return ((private_data_t *)out->priv)->attr.log_level;
}

void pcmk__output_set_log_level(pcmk__output_t *out, int log_level) {
    CRM_ASSERT(out && out->priv);
    ((private_data_t *)out->priv)->attr.log_level = log_level;
}
...
pcmk__output_set_file_func_line(pcmk__output_t *out, const char *filename
    ...
    strncpy(priv->attr.filename, filename, LINE_MAX);
    strncpy(priv->attr.function, function, LINE_MAX);
...
pcmk__output_crm_log(pcmk__output_t *out, const char *format, ...) {
    ...
    strncpy(message->attr.filename, priv->attr.filename, LINE_MAX);
    strncpy(message->attr.function, priv->attr.function, LINE_MAX);

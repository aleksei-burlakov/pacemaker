...
static void
pe__native_output_attr_log(gpointer key, gpointer value, gpointer user_data)
{
    // FIXME! Use out or remove it and pe__data_output_log_s completely
    //pcmk__output_t *out = ((struct pe__data_output_log_s *)user_data)->out;
    int log_level = ((struct pe__data_output_log_s *)user_data)->log_level;
    do_crm_log(log_level, "Option: %s = %s\n", (char *)key, (char *)value);
}
                      ...
                      const char *name, node_t *node, long options, int log_level)
{
    ...
    CRM_LOG_ASSERT(offset > 0);
    if(flagOffset > 0) {
        do_crm_log(log_level, "%s (%s)%s%s", buffer, flagBuffer, desc?" ":"", desc?desc:"");
    } else {
        do_crm_log(log_level, "%s%s%s", buffer, desc?" ":"", desc?desc:"");
    }
    ...
    if ((options & pe_print_rsconly)) {
        ...
        for (; gIter != NULL; gIter = gIter->next) {
            node_t *n = (node_t *) gIter->data;
            counter++;
            do_crm_log(log_level, "\t%d : %s", counter, n->details->uname);
        }
    }

    if (options & pe_print_details) {
        struct pe__data_output_log_s pdata;

        pdata.out = out;
        pdata.log_level = log_level;

        g_hash_table_foreach(rsc->parameters, pe__native_output_attr_log, &pdata);
    }
    ...
    if (options & pe_print_dev) {
        ...
        do_crm_log(log_level, "%s\t(%s%svariant=%s, priority=%f)", pre_text,
                     is_set(rsc->flags, pe_rsc_provisional) ? "provisional, " : "",
                     is_set(rsc->flags, pe_rsc_runnable) ? "" : "non-startable, ",
                     crm_element_name(rsc->xml), (double)rsc->priority);
        do_crm_log(log_level, "%s\tAllowed Nodes", pre_text);
        g_hash_table_iter_init(&iter, rsc->allowed_nodes);
        while (g_hash_table_iter_next(&iter, NULL, (void **)&n)) {
            do_crm_log(log_level, "%s\t * %s %d", pre_text, n->details->uname, n->weight);
        }
    }
    ...
    if (options & pe_print_max_details) {
        ...
        do_crm_log(log_level, "%s\t=== Allowed Nodes\n", pre_text);
        g_hash_table_iter_init(&iter, rsc->allowed_nodes);
        while (g_hash_table_iter_next(&iter, NULL, (void **)&n)) {
            pe__output_node(n, FALSE, out);
        }
...
pe__resource_log(pcmk__output_t *out, va_list args)
{
    ...
    pe__common_output_log(out, rsc, pre_text, rsc_printable_id(rsc), node, options, log_level);
    return 0;
}
...
void
pe__rscs_brief_output_log(pcmk__output_t *out, GListPtr rsc_list, const char *pre_text,
                          long options, gboolean print_all, int log_level)
{
    ...
    while (g_hash_table_iter_next(&hash_iter, (gpointer *)&type, (gpointer *)&rsc_counter)) {
        ...
        while (g_hash_table_iter_next(&hash_iter2, (gpointer *)&node_name, (gpointer *)&node_table)) {
            ...
            if (print_all) {
                do_crm_log(log_level, "%s%d/%d\t(%s):\tActive %s\n", pre_text ? pre_text : "",
                             active_counter ? *active_counter : 0,
                             rsc_counter ? *rsc_counter : 0, type,
                             active_counter && (*active_counter > 0) && node_name ? node_name : "");
            } else {
                do_crm_log(log_level, "%s%d\t(%s):\tActive %s\n", pre_text ? pre_text : "",
                             active_counter ? *active_counter : 0, type,
                             active_counter && (*active_counter > 0) && node_name ? node_name : "");
            }
        }
        ...
        if (print_all && active_counter_all == 0) {
            do_crm_log(log_level, "%s%d/%d\t(%s):\tActive\n", pre_text ? pre_text : "",
                         active_counter_all,
                         rsc_counter ? *rsc_counter : 0, type);
        }
    }

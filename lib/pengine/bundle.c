                              ...
                              const char *pre_text, long options)
{
    ...
    node = pe__current_node(replica->container);
    pe__common_output_log(out, rsc, pre_text, buffer, node, options);
}
...
pe__bundle_log(pcmk__output_t *out, va_list args)
{
    ...
    pcmk__output_do_crm_log(out, "%sContainer bundle%s: %s [%s]%s%s\n",
    ...
    for (GList *gIter = bundle_data->replicas; gIter != NULL;
        ...
        if (is_set(options, pe_print_implicit)) {
            ...
            if(g_list_length(bundle_data->replicas) > 1) {
                pcmk__output_do_crm_log(out, "  %sReplica[%d]\n", pre_text, replica->offset);
            }
            ...
            child_text = crm_strdup_printf("%s  ", pre_text);
            pe__bundle_replica_output_log(out, replica, child_text, options);
        }
        free(child_text);
    }
    return 0;
